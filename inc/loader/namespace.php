...
function is_development() {
	return apply_filters( 'appregistry.loader.is_development', WP_DEBUG );
}
...
function get_assets_list( string $directory ) {
	...
	if ( is_development() ) {
		...
		if ( ! empty( $dev_assets ) ) {
			return array_map(
				function ( $url ) {
					return str_replace( '://0.0.0.0:', '://127.0.0.1:', $url );
				},
